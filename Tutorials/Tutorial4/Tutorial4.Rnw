...
# Now we can create a new column in dec25k that will contain the amount of river edge we have in the buffer of the receiver and put 0. FOr this we first need to make the
dec25k$H2O <- 0
# We can then add for the value to the associated row
