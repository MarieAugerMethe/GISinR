...
# unlik ethe usaKntm object made above this will have data
class(usaKntm)
class(usaKntm2)
summary(usaKntm2)
...
MAYBE ADD HOW TO LINK TO DATA. BUT NEED TO MAKE A UNION THAT KEEPS THE DATA
...
recUTM <- spTransform(recLoc, CRS("+proj=utm +zone=10 +datum=WGS84"))
riversUTM <- spTransform(rivAll, CRS("+proj=utm +zone=10 +datum=WGS84"))
# Lets' look at it
plot(boundsUTM)
plot(riversUTM, add=TRUE, col="blue")
plot(recUTM, pch=19, cex=0.5, col="red", add=TRUE)
@
...
rec25k <- gBuffer(recUTM, width=25000, byid=TRUE)
# We now have a SpatialPolygons
class(riv10k)
class(rec25k)
summary(rec25k)
plot(boundsUTM)
...
plot(recUTM, col="red", add=TRUE, pch=19, cex=0.5)
plot(rec25k, col=rgb(1,0,0,0.5), border=NA, add=TRUE)
@

Now we would like to see how much water edge, represented by the 10k buffer, is in the vacinity (25k buffer) of each receiver. To do this
