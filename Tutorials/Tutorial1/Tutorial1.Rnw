...
<<plot,fig.align="center",out.width="0.5\\linewidth">>=
plot(bioPr)
@
...
It can happen that you may have the coordinated information coming from a different file than the file in which you have information. In which case you can pyt the two together using the \texttt{match.ID} option of \texttt{SpatialPointsDataFrame}... bla bla bla
